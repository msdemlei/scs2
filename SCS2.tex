...
Operators already running ConeSearch-1 services should keep doing so
until the transition team declares the migration window closed, even
while the tables are accessible through SCS2 already.

Ideally, newly implemented SCS2 services would be accompanied by
ConeSearch-1 interfaces as well in order to keep legacy clients viable
for as long as possible.  However, there is no requirement to newly
implement ConeSearch-1 in such a situation.  Here, the desideratum of
publishers not having to implement multiple major versions outweighs the
desideratum that users see the same VO independently of the client they
use.

it is deemed acceptable that these legacy client will
not see all the growth of the VO.  Hence, there are no requirements on
having such interfaces in newly written server software.
